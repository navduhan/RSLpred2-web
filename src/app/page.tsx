...
export default function Home() {
  return (
    <div className="space-y-10 pb-10 pt-4 sm:space-y-14 sm:pt-8">
      <section className="relative overflow-hidden rounded-[2rem] border border-[#cddcd3] bg-[#f7f8f3] px-6 py-10 sm:px-10 sm:py-12 lg:px-14">
        <div className="pointer-events-none absolute -right-24 -top-32 h-96 w-96 rounded-full border-[52px] border-[#dce9df]/60" />
        <div className="pointer-events-none absolute bottom-0 right-[36%] h-24 w-24 translate-y-1/2 rounded-full bg-[#d6a83b]/15" />

        <div className="relative grid gap-10 lg:grid-cols-[1.35fr_0.85fr] lg:items-end">
          <div>
            <p className="mb-5 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-[#136d4b]">
              <span className="h-px w-8 bg-[#136d4b]" />
            ...
            <h1 className="max-w-3xl font-serif text-4xl font-semibold leading-[1.05] tracking-[-0.035em] text-[#082b3b] sm:text-5xl lg:text-6xl">
              ...
              <span className="block italic text-[#136d4b]">resolved for rice.</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href="/prediction"
                className="inline-flex items-center gap-2 rounded-full bg-[#136d4b] px-6 py-3 text-sm font-bold text-white shadow-[0_8px_24px_rgba(19,109,75,0.22)] transition hover:-translate-y-0.5 hover:bg-[#0d583c]"
              >
                Run a prediction <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                href="/download"
                className="inline-flex items-center gap-2 rounded-full border border-[#b8cec0] bg-white/80 px-6 py-3 text-sm font-bold text-[#164735] transition hover:-translate-y-0.5 hover:border-[#136d4b]"
              >
          ...
          <div className="rounded-[1.5rem] border border-white/80 bg-white/75 p-3 shadow-[0_20px_60px_rgba(8,43,59,0.08)] backdrop-blur">
            <div className="flex items-center justify-between px-3 pb-3 pt-2">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#136d4b]">Prediction hierarchy</p>
                <p className="mt-1 text-sm text-slate-500">One sequence, four decisions</p>
              </div>
              <Layers3 className="h-5 w-5 text-[#136d4b]" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              {hierarchy.map((item) => (
                <div key={item.level} className="rounded-2xl border border-[#dce6df] bg-[#fbfcf9] p-4">
                  <span className="text-[11px] font-black uppercase tracking-[0.18em] text-[#a17111]">Level {item.level}</span>
      ...
      <section aria-labelledby="workflow-title">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#136d4b]">From sequence to compartment</p>
        ...
        <figure className="overflow-hidden rounded-[1.75rem] border border-[#d8e2db] bg-[#fbfbf7] p-2 shadow-[0_24px_70px_rgba(8,43,59,0.10)] sm:p-4">
          <Image
            src="/assets/images/rslpred2_workflow_acd.png"
      ...
      <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <article className="rounded-[1.75rem] border border-slate-200 bg-white p-7 sm:p-9">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#136d4b]">Why species-specific?</p>
          <h2 className="mt-3 font-serif text-3xl font-semibold tracking-tight text-[#082b3b]">Built around the rice proteome</h2>
        ...
        <aside className="flex flex-col justify-between rounded-[1.75rem] bg-[#082b3b] p-7 text-white sm:p-9">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#8fd1af]">Ready to analyze?</p>
          ...
          <Link href="/prediction" className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-[#a7e4c5] hover:text-white">
            Open the prediction server <ArrowUpRight className="h-4 w-4" />
          </Link>
        </aside>
      </section>

      <section className="rounded-[1.75rem] border border-[#d8e2db] bg-[#f7f8f5] p-6 sm:p-8" aria-labelledby="citation-title">
        <div className="grid gap-5 lg:grid-cols-[220px_1fr] lg:items-start">
          <div className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-full bg-[#dfece4] text-[#136d4b]">
              <BookOpen className="h-5 w-5" />
            </span>
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#136d4b]">Publication</p>
              <h2 id="citation-title" className="mt-1 font-bold text-[#082b3b]">Please cite RSLpred2</h2>
            </div>
          </div>
          <div className="border-t border-[#d8e2db] pt-5 lg:border-l lg:border-t-0 lg:pl-7 lg:pt-0">
            ...
            <a
              ...
              className="mt-3 inline-flex items-center gap-1.5 text-sm font-bold text-[#136d4b] hover:underline"
            >
